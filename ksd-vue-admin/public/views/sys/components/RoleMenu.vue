...
import { SelectRoleMenus, SaveRoleMenus } from '@/api/sysroles.js'
...
const handleOpen = async (role) => {
    ...
    // 查询当前角色已经授权的菜单
    const resp = await SelectRoleMenus(role.id) // 查询当前角色的菜单
...
const handleSubmitMenusToRole = async () => {
    ...
    const resp = await SaveRoleMenus({ "roleId": roleId, "menuIds": menuIds })
